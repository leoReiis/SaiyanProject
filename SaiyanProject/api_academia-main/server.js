...
app.get("/dashboard", (req, res) => {
  ...
  db.get(countAlunosQuery, [], (errAlunos, resultAlunos) => {
    if (errAlunos) {
      ...
      return res
        .status(500)
        .json({ error: "Erro ao buscar informações do dashboard" });
    }

    db.get(countExerciciosQuery, [], (errExercicios, resultExercicios) => {
      if (errExercicios) {
        console.error(
          "Erro ao buscar contagem de exercícios:",
          errExercicios.message
        );
        return res
          .status(500)
          .json({ error: "Erro ao buscar informações do dashboard" });
...
app.post("/workouts/check", (req, res) => {
  ...
  const query = `
    ...
    VALUES (?, ?, ?)
  `;
  db.run(query, [student_id, workout_id, day_of_week], function (err) {
    if (err) {
      console.error("Erro ao associar treino ao estudante:", err.message);
      return res
        .status(500)
        .json({ error: "Erro ao associar treino ao estudante" });
    }
    console.log(
      "Associação de treino e estudante cadastrada com ID:",
      this.lastID
    );
    res
      .status(201)
      .json({
        message: "Associação de treino e estudante cadastrada com sucesso",
      });
  });
});
